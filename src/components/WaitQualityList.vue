                 ...
                 <table class="table table-bordered" style="text-align: center !important;">
                   ...
                   <tbody>
                     <tr v-for="(item,index) in items">
                       <td>{{ getNum(index) }}</td>
                       <td>
                         <router-link :to="'/financialForm/list2/'+item.orderNum+'/'+item.proposer +'/'+item.status">
                           <a>{{ item.orderNum }}</a>
                         </router-link>
                       </td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initTable : function () {
        var that = this;
        that.$utils.ajax(that)(basePath2+'order/waitCheck',this.searchParams, function (response) {
