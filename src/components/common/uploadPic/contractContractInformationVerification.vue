<template>
  <div>
    <vue-file-upload v-bind:url='url' ref="vueFileUploader" v-bind:filters="filters"
                     v-bind:events='cbEventsCIV' v-bind:request-options="reqopts" v-on:onAdd="onAddItem"
...
<script>
	...
	default {
		...
		computed: {
			url: function () {
				return basePath2 + '/contract/' + this.$route.params.orderNum + '/uploadVerification' + "?accessToken=" + this.$utils.getLocalData("accessToken");
			}
		},
		methods: {
			onStatus(file) {
