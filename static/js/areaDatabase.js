...
function _cityData(proId){
	var options = [];
	$(cities).each(function(i, item){
		if(item.ProID == proId){
			var option = {};
			option.id = item.CityID;
			option.text = item.name;
			options.push(option);
		}
	});
...
function _cityData2(name){
  var proId = _getProIDByName(name);
  var options = [];
  $(cities).each(function(i, item){
    if(item.ProID == proId){
      var option = {};
