...
var Monthpicker = function() {
  ...
  c.prototype.EvaluateOptions = function() {
    ...
    if (null !== this.opts.minValue || null !== this.opts.minYear) if (null !== this.opts.minValue && null !== this.opts.minYear) {
      var b = this.opts.minValue.split("-"),
        ...
        d = parseInt(b[0]);
      c > d ? (a.min.year = c, a.min.month = 1) : (a.min.year = d, a.min.month = parseInt(b[1]))
    } else null !== this.opts.minValue ? (b = this.opts.minValue.split("-"), a.min.year = parseInt(b[0]), a.min.month = parseInt(b[1])) : (a.min.year = parseInt(this.opts.minYear), a.min.month = 1);
    if (null !== this.opts.maxValue || null !== this.opts.maxYear) null !== this.opts.maxValue && null !== this.opts.maxYear ? (b = this.opts.maxValue.split("-"), c = parseInt(this.opts.maxYear), d = parseInt(b[0]), c < d ? (a.max.year = c, a.max.month = 12) : (a.max.year = d, a.max.month = parseInt(b[1]))) : null !== this.opts.maxValue ? (b = this.opts.maxValue.split("-"), a.max.year = parseInt(b[0]), a.max.month = parseInt(b[1])) : (a.max.year = parseInt(this.opts.maxYear), a.max.month = 12);
    this.bounds = a
  };
  c.prototype.RefreshInputs = function() {
    this.selectedYear && this.selectedMonth ? (this.original_input.value = (this.selectedYear + "-" + (10 > this.selectedMonth ? "0" + this.selectedMonth: this.selectedMonth.toString())), this.input.innerHTML = this.selectedYear + " " + this.opts.monthLabels[this.selectedMonth - 1]) : this.input.innerHTML = '<span class="placeholder">' + this.original_input.placeholder + "</span>"
  };
  ...
  c.prototype.InitIU = function() {
    ...
    for (var a = "<table><tr><td><span class='yearSwitch down'>&lt;</span></td><td><div class='yearValue'>" + this.currentYear + "</div> </td><td><span class='yearSwitch up'>&gt;</span> </td></tr> ",
           b = 0; 4 > b; b++) var c = 3 * b,
  ...
  c.prototype.Init = function() {
    ...
    this.parent.addEventListener("blur",
      ...
      !0);
    this.parent.querySelector(".yearSwitch.down").addEventListener("click",
      function() {
        c.instances[this.closest(".monthpicker").dataset.mp].PrevYear()
      });
    this.parent.querySelector(".yearSwitch.up").addEventListener("click",
      function() {
  ...
  c._clone = function(a) {
    ...
    if (a instanceof Date) return b = new Date,
      b.setTime(a.getTime()),
      b;
    if (a instanceof Array) {
      b = [];
      for (var e = 0,
             d = a.length; e < d; e++) b[e] = c._clone(a[e]);
      return b
    }
...
window.Element &&
function(c) {
  c.matches = c.matches || c.matchesSelector || c.webkitMatchesSelector || c.msMatchesSelector ||
    function(a) {
